...
class bacula::director (
  ...
  String $mail_command                      = "/usr/sbin/bsmtp -h localhost -f bacula@${facts['networking']['fqdn']} -s \\\"Bacula %t %e (for %c)\\\" %r",
  ...
  String $operator_command                  = "/usr/sbin/bsmtp -h localhost -f bacula@${facts['networking']['fqdn']} -s \\\"Bacula Intervention Required (for %c)\\\" %r",
  String $var_dir                           = '/var/lib/bacula',
  String $pid_dir                           = $var_dir,
  Optional[String] $plugin_dir              = '/usr/lib64/bacula',
  String $working_dir                       = $var_dir,
  String $storage_server                    = "bacula.${facts['networking']['domain']}",
